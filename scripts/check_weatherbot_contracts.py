...
LOCKFILE_SHA256 = {
    ...
    "requirements-py314.lock": "e58a957a25e974b12618dc2cf8a985aefec02046e4e89dcca062eabb7d7e7fb5",
}
...
def parse_hashed_lock(lock_path):
    text = lock_path.read_text(encoding="utf-8")
    assert_true(
        text.startswith("# This file was autogenerated by uv via the following command:\n#    make lock\n"),
        "{0} must record deterministic make lock provenance".format(lock_path.name),
    )
    assert_true("--index-url" not in text and " @ " not in text, "{0} must not redirect package sources".format(lock_path.name))
    ...
    packages = {}
    for position, index in enumerate(package_indexes):
        line = lines[index]
        name, version_text = line.split("==", 1)
        normalized_name = name.lower().replace("_", "-")
        version = version_text.split()[0].rstrip("\\")
        block_end = package_indexes[position + 1] if position + 1 < len(package_indexes) else len(lines)
        block = "\n".join(lines[index:block_end])
        assert_true("--hash=sha256:" in block, "{0} package {1} must have artifact hashes".format(lock_path.name, normalized_name))
        assert_true(normalized_name not in packages, "{0} must not duplicate {1}".format(lock_path.name, normalized_name))
        packages[normalized_name] = version
    return packages


def test_supported_python_dependency_graphs_are_hash_locked():
    for lock_name, expected_sha256 in LOCKFILE_SHA256.items():
        ...
        assert_equal(actual_sha256, expected_sha256, "{0} complete-file digest".format(lock_name))
        expected_packages = dict(COMMON_LOCKED_PACKAGES)
        if lock_name == "requirements-py314.lock":
            expected_packages["legacy-cgi"] = "2.6.4"
        assert_equal(parse_hashed_lock(lock_path), expected_packages, "{0} package graph".format(lock_name))


def test_runtime_dependencies_and_ci_are_pinned():
    agent_guidance = (ROOT / "AGENTS.md").read_text(encoding="utf-8")
    assert_true(
        "python3 -m pip install --require-hashes -r requirements-py314.lock" in agent_guidance,
        "agent setup guidance must install the deployment lock with hash verification",
    )
    ...
    for contract in [
        ...
        'python -m pip install --require-hashes --requirement "${{ matrix.lockfile }}"',
        "run: make check",
    ]:
    ...
    for python_version, lockfile in (
        ("3.10", "requirements-py310.lock"),
        ("3.12", "requirements-py312.lock"),
        ("3.14", "requirements-py314.lock"),
    ):
        matrix_entry = '          - python-version: "{0}"\n            lockfile: {1}'.format(
            python_version, lockfile
        )
        assert_true(matrix_entry in workflow, "hosted matrix must bind Python {0} to {1}".format(python_version, lockfile))
    ...
    assert_true("UV ?= uv" in makefile_lines, "Makefile must preserve the uv command override")
    lock_command_template = (
        'cd "$(ROOT)" && $(UV) pip compile requirements.txt test-requirements.txt '
        '--python-version {python_version} --python-platform x86_64-manylinux_2_28 '
        '--default-index https://pypi.org/simple --generate-hashes '
        "--custom-compile-command 'make lock' --output-file {lockfile}"
    )
    for python_version, lockfile in (
        ("3.10", "requirements-py310.lock"),
        ("3.12", "requirements-py312.lock"),
        ...
        assert_true(
            ...
            "Makefile must preserve deterministic {0} lock generation".format(python_version),
        )
    assert_equal(makefile.count("$(UV) pip compile requirements.txt test-requirements.txt"), 3, "lock command count")
    documentation_contracts = {
        "README.md": (
            "complete SHA-256-locked dependency graphs",
            "make lock",
            "matrix-matched hash-locked dependency graph",
            "Use `requirements-py310.lock` or `requirements-py312.lock` instead",
        ),
        "SECURITY.md": (
            "pip\n`--require-hashes`",
            "accepted distribution artifacts",
        ),
        "CHANGES.md": (
            "complete SHA-256-locked Python 3.10, 3.12, and 3.14 dependency graphs",
            "deterministic\n  lock regeneration and integrity contracts",
        ),
    }
    for relative_path, phrases in documentation_contracts.items():
        text = (ROOT / relative_path).read_text(encoding="utf-8")
        for phrase in phrases:
            assert_true(phrase in text, "{0} must document {1}".format(relative_path, phrase))
